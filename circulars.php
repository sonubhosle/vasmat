...
// Build query
$sql_faculty_circulars = "SELECT fc.id, fc.title, fc.description, fc.file_path, f.name as uploaded_by, fc.status, fc.created_at, fc.department 
                        FROM faculty_content fc 
                        JOIN faculty f ON fc.faculty_id = f.id 
                        WHERE fc.type = 'circulars' AND fc.status = 'approved'";

$final_sql = "SELECT * FROM ($sql_faculty_circulars) as combined WHERE 1=1";
...
$stmt->execute();
$circulars = $stmt->get_result();
$total_rows = $circulars->num_rows;
...
<div class="px-6 py-4 sm:py-6 lg:py-8 max-w-7xl mx-auto">
    ...
    <div class="mb-12 animate-fade-in-up">
        <div class="flex items-center gap-4 mb-4">
            <span class="text-[10px] font-black uppercase tracking-[0.3em] text-rose-500">Official Communication</span>
        </div>
        <h2 class="text-2xl md:text-3xl font-black text-slate-900 mb-6">
            Important <span class="italic font-serif">Circulars</span>
        </h2>
        <p class="text-slate-500 text-lg max-w-2xl">Official notices and departmental circulars issued by the faculty and college administration.</p>
    </div>
    ...
    <div class="glass-card rounded-[2rem] p-6 mb-12 shadow-xl shadow-slate-200/50 animate-fade-in-up" 
        ...
        <form method="GET" action="" class="space-y-6">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="relative">
                    ...
                    <input type="text" 
                           ...
                           class="w-full py-3.5 pl-12 pr-4 rounded-2xl border border-slate-100 bg-slate-50/50 focus:outline-none focus:ring-4 focus:ring-rose-400/10 focus:border-rose-400 transition-all font-medium">
                </div>
            ...
            <div class="flex justify-end gap-3 pt-2">
                <a href="circulars.php" class="px-6 py-3 border border-slate-200 text-slate-500 font-bold rounded-2xl hover:bg-slate-50 transition-all flex items-center gap-2 text-sm uppercase tracking-widest">
    ...
    <div class="animate-fade-in-up" style="animation-delay: 0.2s; position: relative; z-index: 1;">
        <?php if ($total_rows == 0): ?>
            <div class="text-center py-20 glass-card rounded-[2rem] border-2 border-dashed border-slate-200">
                ...
                <h3 class="text-2xl font-black text-slate-800 mb-2">No Circulars Found</h3>
                <p class="text-slate-500 mb-8">No official circulars have been uploaded for your criteria yet.</p>
                <a href="circulars.php" class="px-8 py-3 bg-rose-500 text-white font-bold rounded-2xl hover:bg-rose-600 transition-all uppercase tracking-widest text-sm">Clear Filters</a>
            </div>
        <?php else: ?>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50/50 border-b border-slate-100">
                                <th class="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Circular Title</th>
                        ...
                        <tbody>
                            <?php while($row = $circulars->fetch_assoc()): ?>
                            <tr class="table-row-hover transition-all duration-300 group">
                                <td class="px-8 py-6">
                                    <div class="flex items-center gap-5">
                                        <div class="w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 bg-rose-50 text-rose-500 shadow-sm transition-transform group-hover:scale-110">
                                            <i class="fas fa-file-contract text-xl"></i>
                                        </div>
                                        <div>
                                            <h4 class="text-[17px] font-black text-slate-800 tracking-tight group-hover:text-rose-600 transition-colors"><?= htmlspecialchars($row['title']) ?></h4>
                                ...
                                <td class="px-8 py-6">
                                    <div class="flex justify-end gap-3">
                                        <a href="<?= htmlspecialchars($row['file_path']) ?>" 
                                           target="_blank"
                                           class="w-10 h-10 rounded-xl bg-slate-100 text-slate-500 flex items-center justify-center hover:bg-rose-500 hover:text-white hover:scale-110 transition-all shadow-sm"
                                           title="Preview">
